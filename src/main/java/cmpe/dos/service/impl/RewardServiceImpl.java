...
public class RewardServiceImpl implements RewardService {
	...
	public Boolean addNewCoupon(Integer years, Integer times, Float values, Integer duration) {
		...
		// List users = userDao.doQueryList("select b.USERNAME from USER as a, ORDERS as b where a.USERNAME = b.USERNAME " +
		//"and floor(datediff(curdate(), a.SIGNUP_DATE)/365) > ? group by b.USERNAME having count(*) > ?;", true, today, years, times);

		for(int i = 0; i < users.size(); i++) {
			Reward reward = new Reward();
			reward.setCouponId("loyaltyReward");
			reward.setUsername((String)((Object[])(users.get(i)))[0]);
